...
function regNewOrder($conn){
   ...
   $username= $_SESSION['my'];
    $query="INSERT INTO `onlyone`(`name`, `descrip`,`price`,`user`,`namePerson`,`phone`, `type`) VALUES
    ('$name','$descrip','$price','$username','$namePerson','$phone','$type') ";
...
function searchSelling($conn){
    ...
    $messageArray=array();
    $data=array();
    $query="SELECT * FROM `pyments` WHERE `id` LIKE '%$val%'  OR `namePerson`  LIKE '%$val%' OR `phone` LIKE '%$val%' OR `name` LIKE '%$val%' AND type='selling'";
    $result=$conn->query($query);
    if ($result) {
...
function searchAll($conn){
    ...
    $messageArray=array();
    $data=array();
    $query="SELECT * FROM `pyments` WHERE `id` LIKE '%$val%'  OR `namePerson`  LIKE '%$val%' OR `phone` LIKE '%$val%' OR `name` LIKE '%$val%' ";
    $result=$conn->query($query);
    if ($result) {
        while ($row=$result->fetch_assoc()) {
            $data[]=$row;
...
function regpayment($conn){
    ...
    $messageArray=array();
   $username= $_SESSION['my'];
    $query="INSERT INTO `pyments`(`name`, `model`, `size`, `color`, `descrip`, `price`, `user`, `namePerson`, `phone`, `type`, `peyMethod`) VALUES
    ('$name','0','0','0','$descrip','$price','$username','$namePerson','$phone','$type','$peyMethod') ";
    $result=$conn->query($query);
   if ($result) {
...
function  listAllSelling($conn){
    ...
    $messageArray=array();
    $data=array();
    $query="SELECT * FROM `pyments` WHERE `type`='selling'";
    $result=$conn->query($query);
    if ($result) {
        while ($row=$result->fetch_assoc()) {
            $data[]=$row;
...
function updatepayment($conn){
    ...
    // $username= $_SESSION['my'];
    $messageArray=array();
    $query="UPDATE `pyments` SET `id`='$id', `name`='$name',`descrip`='$descrip',`price`='$price',`namePerson`='$namePerson',`phone`='$phone',`type`='selling',`peyMethod`='$peyMethod' WHERE `id`='$id'";
    $result=$conn->query($query);
    if ($result) {
...
function deleteAllSelling($conn){
    extract($_POST);
    $messageArray=array();
    $query="DELETE FROM `pyments` WHERE `type`='selling'";
    $result=$conn->query($query);
    if ($result) {
